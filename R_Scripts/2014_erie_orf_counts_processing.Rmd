---
title: "2014 Erie ORF counts processing"
author: "A.J Wing"
date: '2022-03-24'
...
```{r}
pal <- ggthemes::tableau_color_pal(palette="Traffic", type="regular")
```
...
```{r}
png("~/Desktop/erie_story_r_work/top_pfam_proportion.png", width=6, height=10, units="in", res=300)
pfam_abund     
dev.off()
```

```{r}
pdf("~/Desktop/erie_story_r_work/top_pfam_proportion.pdf", width=8, height=8)
pfam_abund     
dev.off()
```

```{r}
head(sort(table(annotation_coverage_pfam$vogdb_hit), decreasing=T), n=20)
```
...
```{r}
ko_abund <- ggplot(orf_summary[1:10,], 
...
kegg_metabolism <- readLines("~/Desktop/erie_story_r_work/kegg_metabolism.txt")
...
```{r}
orf_summary_high <- orf_summary[orf_summary$Proportion_KO>0.001 | orf_summary$Proportion_Abund>0.001,]
...
```{r}
level2_summary$numKOs_database <- table(kegg_metabolism$Level2)[names(table(kegg_metabolism$Level2)) %in% level2_summary$PathwayModule]

level2_summary$databaseproportion <- level2_summary$numKOs_database/nrow(kegg_metabolism[kegg_metabolism$Level2 %in% level2_summary$PathwayModule,])
```


```{r}
ggplot(level2_summary, 
...
```{r}
ko_abund <- ggplot(level2_summary, 
                        aes(x=Proportion_Abund, y=PathwayModule,
...
annotation_kegg <- annotation_kegg %>%
  pivot_wider(names_from=Assembly, values_from=Counts, values_fill=0)
```
